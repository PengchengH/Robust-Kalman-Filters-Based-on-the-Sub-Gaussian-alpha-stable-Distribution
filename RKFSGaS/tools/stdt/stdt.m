...
%         frdn
    ...
    methods   
       function obj = stdt()  
           ...
           obj.name  = 'stdt';
           obj.Ke    = @(X_in) X_in;
           obj.fmrdn = @(v_in,s1_in,s2_in) gamrnd(v_in/2,2,s1_in,s2_in)./v_in;
            obj.fpdf  = @(X_in, mu_in, C_in, df_in) stpdf(X_in, mu_in, C_in, df_in);
%           obj.frdn  = @(mu_in,C_in,df_in,cases_in) stdtrnd(mu_in',C_in,df_in,cases_in)';
       end 
    ...
    methods 
      ...
      function [out] = opt(~,n,dof,eta1)
          ...
          out    = struct;
          out.EK = (n+dof)/(eta1+dof);
      end
      
      function [r,scale] = frnd(~,mu,C,df,cases)
