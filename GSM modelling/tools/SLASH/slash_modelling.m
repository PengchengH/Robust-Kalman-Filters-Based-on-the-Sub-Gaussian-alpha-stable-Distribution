classdef slash_modelling
    ...
    function obj = slash_modelling()  
        ...
        obj.model.fmrdn = @(v_in,s1_in,s2_in) random('Beta',v_in/2,1,[s1_in,s2_in]);
        obj.model.fpdf  = @(X_in, mu_in, C_in, df_in) slashpdf(X_in, mu_in, C_in, df_in);
        obj.model.frdn  = @(mu_in,C_in,df_in,cases_in) slashrnd(mu_in,C_in,df_in,cases_in);
...
%         SIGMA(:,:,t+1) = 0.5;
        if sum(SIGMA(:,:,t+1))> sum(SIGMA(:,:,t))*1.05
            SIGMA(:,:,t+1) = SIGMA(:,:,t)*1.05;
        elseif sum(SIGMA(:,:,t+1))< sum(SIGMA(:,:,t))*0.95
            SIGMA(:,:,t+1) = SIGMA(:,:,t)*0.95;
        end

        %%  Update the dof estimation;                         
        loss_1 = @(dof_in) -sum(log(fpdf(Y,mu(:,t+1),SIGMA(:,:,t+1),dof_in)));
        dof(t+1) = linear_search(loss_1,XL,XU,epsilon,iteration);
        
        if dof(t+1) > dof(t)*1.1
            dof(t+1) = dof(t)*1.1;
        elseif dof(t+1) < dof(t)*0.9
            dof(t+1) = dof(t)*0.9;
        end
