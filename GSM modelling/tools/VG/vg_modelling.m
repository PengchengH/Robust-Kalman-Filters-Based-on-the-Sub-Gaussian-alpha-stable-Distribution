classdef vg_modelling
    ...
    function obj = vg_modelling()  
        ...
        obj.model.fmrdn = @(v_in,s1_in,s2_in) 1./gamrnd(v_in/2,2/v_in,s1_in,s2_in);
        obj.model.flogpdf  = @(X_in, mu_in, C_in, df_in) vglogpdf(X_in, mu_in, C_in, df_in);
        obj.model.frdn   = @(mu_in,C_in,df_in,cases_in) vgrnd(mu_in,C_in,df_in,cases_in);
    ...
    function para = fitting(obj,Samp,model_name)
        ...
        elseif strcmp(model_name,'vg_moment')
            para = moment_fitting(obj,Samp);
        end
    end
    
    function [para] = moment_fitting(obj,Samp)
        d     = size(Samp,1);
        Y     = Samp;
        if  d>1
        ...
        para.mu    = mu;
        para.SIGMA = ez1;
        para.dof   = 2*ez1^2/(ez2-ez1^2);
                      
        para.dof = max(para.dof,0);
        para.SIGMA = max(para.SIGMA,0);
        
    end
    
    function [para] = MLE_fitting(obj,Samp)
        flogpdf  = obj.model.flogpdf;
        d     = size(Samp,1);
        Y     = Samp;
        if  d>1
        error('dimension shoule be 1');
        ...
        [para_1] = moment_fitting(obj,Samp);
        
        mu     = para_1.mu;
        SIGMA  = para_1.SIGMA;
        dof    = para_1.dof;
        
        loss  = @(x) -sum(flogpdf(Y,mu,x(1),x(2)));
        x0    = [SIGMA,dof];
        lb    = [0 0];
        ub    = [Inf  Inf]; 
