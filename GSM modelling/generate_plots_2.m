...
SSGn    = struct;
SSGn.Q  = Qn;
SSGn.dof_index = [0.3 0.5 0.7 0.9 1.1 1.3 1.5 1.7 1.85];
...
for noise_c = 1:size(noise_index,2)
    if strcmp(noise_index{noise_c},'gm')
        tail_v = GMn.U_index(tail_c);
    elseif strcmp(noise_index{noise_c},'stdt')
        tail_v = stdtn.dof_index(tail_c);
    elseif strcmp(noise_index{noise_c},'ssg')
        tail_v = SSGn.dof_index(tail_c);
    end
for model_c = 1:size(model_index,2)
 
if isfile(['result_1/' model_index{model_c} '_' noise_index{noise_c} '_tail_' num2str(tail_v) ...
    '.mat'])

load(['result_1/' model_index{model_c} '_' noise_index{noise_c} '_tail_' num2str(tail_v) ...
       '.mat']);
    ...
    if isfield(ns,'U')
        save(['result_2/' 'md_' model_index_1{model_c} '_ns_' ...
...
%     hold on;
    semilogy(1:1:num_tail,abs(delta_mean(:,noise_c,model_c)),'-',"Color","#D95319",'LineWidth',3);
%     hold off;
    title([noise_index{noise_c} '&' model_index{model_c}]);
end  
end
% legend('mean','Orientation','horizontal','FontSize',12,'Location',[0.4 0.03 0.2 0]);
...
% %% display
% figure(1);
% for q_c = 1:size(q_index,2)
% for n_c = 1:size(n_index,2)
%     subplot(size(n_index,2),size(q_index,2),(n_c-1)*size(q_index,2)+q_c);
%     semilogy(dof_index,abs(dof_data_error(:,q_c,n_c,1)),'--r','LineWidth',3);
%     hold on;
%     semilogy(dof_index,abs(dof_data_error(:,q_c,n_c,2)),'--b','LineWidth',3);
%     semilogy(dof_index,abs(dof_data_error(:,q_c,n_c,3)),'--m','LineWidth',3);
%     semilogy(dof_index,abs(dof_data_error(:,q_c,n_c,4)),'--g','LineWidth',3);
%     semilogy(dof_index,abs(dof_data_error(:,q_c,n_c,5)),'--y','LineWidth',3);
%     hold off; 
%     title(['dof-' 'q' num2str(q_index(q_c)) 'n'  num2str(n_index(n_c))]);
% end
% end
%   h = legend('MLE','EM','Mellin','ECF','Percentile','Orientation','horizontal','Location','south');
%   set(h,'FontSize',14);
% 
% figure(2);
% for q_c = 1:size(q_index,2)
% for n_c = 1:size(n_index,2)
%     subplot(size(n_index,2),size(q_index,2),(n_c-1)*size(q_index,2)+q_c);
%     semilogy(dof_index,abs(sigma_data_error(:,q_c,n_c,1)),'--r','LineWidth',3);
%     hold on;
%     semilogy(dof_index,abs(sigma_data_error(:,q_c,n_c,2)),'--b','LineWidth',3);
%     semilogy(dof_index,abs(sigma_data_error(:,q_c,n_c,3)),'--m','LineWidth',3);
%     semilogy(dof_index,abs(sigma_data_error(:,q_c,n_c,4)),'--g','LineWidth',3);
%     semilogy(dof_index,abs(sigma_data_error(:,q_c,n_c,5)),'--y','LineWidth',3);
%     hold off;
%     title(['Sigma-' 'q' num2str(q_index(q_c)) 'n'  num2str(n_index(n_c))]);
%     
% end
% end
%   h = legend('MLE','EM','Mellin','ECF','Percentile','Orientation','horizontal','Location','south');
